...
import { ArrowLeft, Clock, ShieldAlert, CheckCircle, XCircle, Briefcase, Hammer, MapPin, Hourglass, CircleDollarSign } from 'lucide-react';
  ...
  if (!task) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6 max-w-md mx-auto bg-white min-h-screen border-x border-border text-center">
        ...
        <p className="text-sm text-muted mt-2">Task details could not be found.</p>
        <Link
          href="/worker/today"
    ...
    <div className="flex-1 flex flex-col max-w-md mx-auto bg-[#faf9f5] min-h-screen border-x border-border shadow-sm pb-8">
      ...
      <div className="p-5 flex flex-col gap-5">
        ...
        <div className="bg-white border border-border p-5 rounded-3xl shadow-sm flex items-center gap-3 text-xs text-muted">
          ...
          <span>Coordinates: {task.location.lat.toFixed(4)}, {task.location.lng.toFixed(4)} &bull; Downtown LA zone</span>
        </div>

        {/* Claim CTA — gated by status / flags */}
        {task.status !== 'open' ? (
          ...
          <div className="bg-amber-50 border border-amber-200/80 p-5 rounded-3xl flex gap-3 text-amber-900 shadow-sm mt-2">
            <Hourglass size={22} className="text-amber-700 shrink-0 mt-0.5" />
            <div className="flex flex-col gap-1 text-xs">
              <span className="font-bold text-sm">Coming soon</span>
              <p className="leading-relaxed text-amber-800">This task is not open for claiming yet.</p>
            </div>
          </div>
        ) : task.isFundingNeeded ? (
          <div className="bg-amber-50 border border-amber-200/80 p-5 rounded-3xl flex gap-3 text-amber-900 shadow-sm mt-2">
            <CircleDollarSign size={22} className="text-amber-700 shrink-0 mt-0.5" />
            <div className="flex flex-col gap-1 text-xs">
              <span className="font-bold text-sm">Needs funding</span>
              <p className="leading-relaxed text-amber-800">This task needs funding before it can be claimed.</p>
            </div>
          </div>
        ) : (
          <Link
